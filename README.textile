...
class LongTasks
  ...
  attr_reader :which_queue

  def call_an_instance_method
    # Some other code
  end
  handle_asynchronously :call_an_instance_method, :queue => Proc.new {|i| i.which_queue }
end
</pre>
...
If there are certain types of jobs that you want to have processed first, or have more workers dedicated to, you can put jobs in a queue.

<pre>
Notifier.delay(:queue => 'mailers').signup(@user)
</pre>

You can then have workers process only jobs in specific queues.

<pre>
$ script/delayed_job -q mailers,expirations start
$ QUEUE=mailers,expirations rake jobs:work
</pre>

If you specify queues when starting a worker, then it will only process jobs in those queues. To simply give those queues priority, but still process other queues, use "*":

<pre>
$ script/delayed_job -q mailers,* start
</pre>
